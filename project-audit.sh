...
SEARCH_ITEM_RE = re.compile(r'^-\s+Read\s+`([^`]+)`\s+(\d+)×')
...
session_search  = defaultdict(list)   # path -> [(file, count)]  — source files
        ...
        for line in f:
            ...
            if current_section and line.startswith('- '):
                if 'new skills to consider' in current_section:
                    ...
                    am = SEARCH_ITEM_RE.match(line)
                    if am:
                        session_search[rpath].append((am.group(1), int(am.group(2))))
                    session_kinds[rpath].add('search')
...
# ── aggregate code-search candidates (source files) across sessions ─────────────
search_sessions = defaultdict(set)   # file -> set of report paths
search_reads    = Counter()          # file -> sum of read counts

for rpath, items in session_search.items():
    for fname, cnt in items:
        search_sessions[fname].add(rpath)
        search_reads[fname] += cnt

recurring_search = [
    (f, len(search_sessions[f]), search_reads[f])
    for f in search_sessions
    if len(search_sessions[f]) >= 2
]
recurring_search.sort(key=lambda x: (-x[1], -x[2]))

single_search = [
    (f, 1, search_reads[f])
    for f in search_sessions
    if len(search_sessions[f]) == 1
]
single_search.sort(key=lambda x: -x[2])
...
KIND_LABELS = {
    ...
    'search':  'Source files (use code search)',
...
# Code-search candidates (source code files)
lines.append("## Better search tool for source files")
lines.append("")
if recurring_search:
    lines.append("Source files read repeatedly across 2+ sessions — use semantic code search for lookups instead of re-reading:")
    lines.append("")
    lines.append("| File | Sessions | Total reads |")
    lines.append("|---|---:|---:|")
    for fname, nsess, nreads in recurring_search:
        lines.append(f"| `{fname}` | {nsess} | {nreads} |")
    lines.append("")
    lines.append("**Action:** Use semantic code search with a natural-language query instead of re-reading these files; one call replaces a chain of Read/Grep calls.")
    lines.append("")
elif single_search:
    lines.append("_No source files read across 2+ sessions. Single-session candidates:_")
    lines.append("")
    for fname, nsess, nreads in single_search[:5]:
        lines.append(f"- `{fname}` — read {nreads}× in 1 session")
    lines.append("")
else:
...
if do_write:
    ...
    log_line = (
        ...
        f"{len(recurring_search)} search candidate(s)  "
        f"-> {audit_path}\n"
    )
